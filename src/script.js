...
function positionStopElement() {
    const arcCenterXinPixels = (arcCenterX / cameraWidth) * window.innerWidth;
    stopElement.style.cssText = `
    left: ${window.innerWidth / 2 }px;
    top: ${window.innerHeight / 2 - arcCenterXinPixels * 0.25}px
  `;
}
...
function renderMap(mapWidth, mapHeight) {
    ...
    //mere
    if (config.trees) {
        const tree1 = Tree("#BF4F3B");
        tree1.position.x = arcCenterX * 1.3;
        scene.add(tree1);

        const tree2 = Tree("#ABBF3B");
        tree2.position.y = arcCenterX * 1.9;
        tree2.position.x = arcCenterX * 1.3;
        scene.add(tree2);

        const tree3 = Tree("#BF4F3B");
        tree3.position.x = arcCenterX * 0.8;
        tree3.position.y = arcCenterX * 2;
        scene.add(tree3);

        const tree4 = Tree("#ABBF3B");
        tree4.position.x = arcCenterX * 1.8;
        tree4.position.y = arcCenterX * 2;
        scene.add(tree4);

        const tree5 = Tree("#BF4F3B");
        tree5.position.x = -arcCenterX * 1;
        tree5.position.y = arcCenterX * 2;
        scene.add(tree5);

        const tree6 = Tree("#ABBF3B");
        tree6.position.x = -arcCenterX * 2;
        tree6.position.y = arcCenterX * 1.8;
        scene.add(tree6);

        const tree7 = Tree("#BF4F3B");
        tree7.position.x = arcCenterX * 0.8;
        tree7.position.y = -arcCenterX * 2;
        scene.add(tree7);

        const tree8 = Tree("#ABBF3B");
        tree8.position.x = arcCenterX * 1.8;
        tree8.position.y = -arcCenterX * 2;
        scene.add(tree8);

        const tree9 = Tree("#BF4F3B");
        tree9.position.x = -arcCenterX * 1;
        tree9.position.y = -arcCenterX * 2;
        scene.add(tree9);

        const tree10 = Tree("#ABBF3B");
        tree10.position.x = -arcCenterX * 2.6;
        tree10.position.y = -arcCenterX * 1.8;
        scene.add(tree10);

        const tree11 = Tree("#BF4F3B");
        tree11.position.x = -arcCenterX * 3;
        tree11.position.y = 0;
        scene.add(tree11);

        const tree12 = Tree("#BF4F3B");
        tree12.position.x = -arcCenterX * -3;
        tree12.position.y = -arcCenterX * 0;
        scene.add(tree12);

        const tree13 = Tree("#ABBF3B");
        tree13.position.x = -arcCenterX * -2.8;
        tree13.position.y = -arcCenterX * 1.5;
        scene.add(tree13);

        const tree14 = Tree("#BF4F3B");
        tree14.position.x = -arcCenterX * 1.5;
        tree14.position.y = -arcCenterX * 1.8;
        scene.add(tree14);

        const tree15 = Tree("#ABBF3B");
        tree15.position.x = -arcCenterX * 2.8;
        tree15.position.y = -arcCenterX * 0.9;
        scene.add(tree15);

        const tree16 = Tree("#BF4F3B");
        tree16.position.x = -arcCenterX * 2.8;
        tree16.position.y = -arcCenterX * -1;
        scene.add(tree16);

        const tree17 = Tree("#BF4F3B");
        tree17.position.x = -arcCenterX * -2.8;
        tree17.position.y = -arcCenterX * -1;
        scene.add(tree17);

        const stopSign1 = stopSign();
        stopSign1.position.x = 0;
        stopSign1.position.y = 0;
        scene.add(stopSign1);
    }
}
...
function stopSign() {
    const tree = new THREE.Group();
    const stopSignGeometry = new THREE.BoxBufferGeometry(5, 30, 30);
    const stopSignMaterial = new THREE.MeshLambertMaterial({
        color: "#71797E"
    });
    const sign = new THREE.Mesh(stopSignGeometry, stopSignMaterial);
    sign.position.z = 10;
    ...
    const crown = new THREE.Mesh(
        new THREE.SphereGeometry(height / 4, 30, 30),
        new THREE.MeshLambertMaterial({
            color: "#FF0000",
        })
    );
